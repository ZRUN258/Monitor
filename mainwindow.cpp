...
QWidget *MainWindow::createOverviewPage() {
    ...
    auto *airContent = new QWidget;
    auto *airLayout = new QVBoxLayout(airContent);
    airLayout->setContentsMargins(0, 0, 0, 0);
    m_airChart = new BarChartWidget;
    airLayout->addWidget(m_airChart, 1);
    auto *airFooter = new QHBoxLayout;
    airFooter->addWidget(mutedLabel(QStringLiteral("当前 TVOC")));
    m_airValue = new QLabel(QStringLiteral("-- ppb"));
    m_airValue->setObjectName(QStringLiteral("airValue"));
    airFooter->addWidget(m_airValue);
    airFooter->addStretch();
    airFooter->addWidget(mutedLabel(QStringLiteral("AGS02MA · 实时")));
    airLayout->addLayout(airFooter);
    auto *airPanel = createPanel(QStringLiteral("空气质量"), airContent);
...
void MainWindow::applyTheme() {
    setStyleSheet(QStringLiteral(R"(
        ...
        QLabel#brand { color: #FFFFFF; font-size: 17px; font-weight: 800; letter-spacing: 2px; padding: 3px; }
        QLabel#brandTag, QLabel#smallCaps, QLabel#eyebrow { color: #00DDF9; font-size: 9px; font-weight: 700; letter-spacing: 2px; }
        QFrame#divider { color: #1A2D4B; background: #1A2D4B; max-height: 1px; border: none; }
        QPushButton#navButton { color: #8B98B7; background: transparent; border: none; border-radius: 11px; padding: 12px 14px; text-align: left; font-size: 13px; font-weight: 600; }
        ...
        QLabel#smallText { color: #B8C4DB; font-size: 11px; }
        QLabel#pageTitle { font-size: 26px; font-weight: 800; color: #FFFFFF; }
        QLabel#clock { font-size: 18px; font-weight: 700; color: #FFFFFF; }
        QLabel#muted { color: #8B98B7; font-size: 11px; }
        QFrame#connectionBar, QFrame#motionBar { background: #0B1B32; border: 1px solid #173255; border-radius: 13px; }
        QLabel#statusPill { color: #FFD166; background: #2A2531; border: 1px solid #6B5738; border-radius: 10px; min-height: 25px; font-size: 11px; font-weight: 700; }
        QLabel#protocolTag, QLabel#liveTag { color: #00E5FF; background: #0A3045; border: 1px solid #14596B; border-radius: 8px; padding: 5px 9px; font-size: 9px; font-weight: 700; }
        ...
        QLabel#metricTitle { color: #96A3BF; font-size: 11px; font-weight: 600; }
        QLabel#metricValue { color: white; font-size: 27px; font-weight: 800; }
        QLabel#metricUnit { color: #8B98B7; font-size: 10px; padding-bottom: 5px; }
        QLabel#panelTitle { color: white; font-size: 14px; font-weight: 700; }
        QLabel#statValue { color: #00E5FF; font-size: 24px; font-weight: 800; }
        QLabel#statValueSmall { color: #FFFFFF; font-size: 17px; font-weight: 700; }
        QLabel#motionTitle { color: #C5D0E5; font-size: 12px; font-weight: 700; }
        QLabel#motionHero { color: #EAF1FF; font-size: 18px; font-weight: 800; }
        QLabel#vectorValue { color: #FFFFFF; font-size: 24px; font-weight: 800; line-height: 1.4; }
        QLabel#vectorValueSmall { color: #FFFFFF; font-size: 18px; font-weight: 700; }
        QLabel#chartValue { color: #FFFFFF; font-size: 13px; font-weight: 800; margin-right: 14px; }
        QLabel#airValue { color: #2CD9FF; font-size: 22px; font-weight: 800; }
        QScrollArea#overviewScroll, QScrollArea#overviewScroll > QWidget > QWidget { background: transparent; border: none; }
        QLineEdit { color: white; background: #08172C; border: 1px solid #214469; border-radius: 9px; padding: 10px 12px; selection-background-color: #087EA4; }
        QLineEdit:focus { border-color: #00DDF9; }
        QPushButton#primaryButton { color: #03111F; background: #00DDF9; border: none; border-radius: 9px; padding: 11px 22px; font-weight: 800; }
        QPushButton#primaryButton:hover { background: #57ECFF; }
        QLabel#hintBox { color: #9AA8C2; background: #0F2544; border: 1px solid #1D3A60; border-radius: 12px; padding: 16px; }
        QTableWidget#dataTable, QListWidget#logList { color: #DDE7F8; background: #081427; alternate-background-color: #0D203B; border: 1px solid #173255; border-radius: 13px; gridline-color: #173255; selection-background-color: #124C70; outline: none; }
        QTableWidget#dataTable::item { padding: 7px; }
        QHeaderView::section { color: #8FA0BD; background: #0D203B; border: none; border-bottom: 1px solid #244261; padding: 10px; font-weight: 700; }
        ...
        QListWidget#timelineList { color: #C9D4E8; background: transparent; border: none; outline: none; }
...
void MainWindow::updateConnectionState() {
    ...
    if (!m_clients.isEmpty()) {
        ...
        m_connectionPill->setStyleSheet(QStringLiteral("color:#01D7A7;background:#0C3836;border:1px solid #17685B;border-radius:10px;min-height:25px;font-size:11px;font-weight:700;"));
        ...
        m_connectionPill->setStyleSheet(QStringLiteral("color:#FF6B89;background:#371C30;border:1px solid #69314C;border-radius:10px;min-height:25px;font-size:11px;font-weight:700;"));
...
void MainWindow::updateMetric(const QString &sensor, const QString &field, double value, qint64 timestamp) {
    if (sensor == QStringLiteral("dht") && field == QStringLiteral("temperature")) {
        ...
        if (m_airChart) m_airChart->append(value);
